...
and is_numeric($_POST['id'])) {
 ...
 if (array_search($ip,explode(",", $file_ip))!==FALSE) {
     $message="<b>You have already voted!</b><br>";
 }
    ...
    $file_ip.=$ip.',';
    fwrite($fh_ip,$ip.',');
    $message="<b>Your vote has been counted !</b><br>";
 }
 $view_res=1;
 flock($fh_ip,LOCK_UN);
 fclose($fh_ip);
...
echo '<form method="POST" style="margin:0 0 0 35px;">Would you like to visit Paris ?
    <input type="hidden" name="id" value="1">';
echo '<table border="0"><tr><td><input type="radio" name="item" value="0" checked>
    Yes</td><td>'.$file[0].'</td></tr>';
echo '<tr><td><input type="radio" name="item" value="1">No</td><td>'.
    $file[1].'</td></tr>';
echo '<tr><td><input type="radio" name="item" value="2">Maybe</td><td>'.
    $file[2].'</td></tr>';
echo '<tr><td colspan="2"><input type="submit" name="view_res" value="Result">
